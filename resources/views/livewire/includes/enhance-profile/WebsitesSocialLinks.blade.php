<section class="form-section rounded">
    ...
    <div id="websitesLinks" class="collapse show">
        ...
        @foreach ($WLForm->websites as $index => $website)
            <div class="row mb-3">
                <div class="form-group col-md-4">
                    <label for="website_name_{{ $index }}">{{ __('general.website_name') }}</label>
                ...
                <div class="form-group flex-grow-1">
                    <label for="link_{{ $index }}">{{ __('general.link') }}</label>
                    <div class="d-flex">
                        ...
                        @if ($index > 0)
                        <i class="bi bi-trash-fill btn rounded ms-1" wire:click="removeRow({{ $index }})"
                            title="{{ __('general.remove_link') }}"></i>
                        @endif
                    </div>
        ...
        <div class="d-flex justify-content-between align-items-center mt-0">
            <button type="button" class="btn btn-primary rounded" wire:click="addRow">
                {{ __('general.add_link') }}
            </button>
        </div>
    </div>
