            ...
            @foreach ($WLForm->websites as $index => $website)
                <div class="form-row">
                    <div class="form-group col-md-4">
                        ...
                        <input type="text"
                            class="form-control @error("WLForm.websites.{$index}.website_name") is-invalid @enderror"
                            id="website_name_{{ $index }}" wire:model="WLForm.websites.{{ $index }}.website_name"
                    ...
                    <div class="form-group flex-grow-1">
                        ...
                        <div class="d-flex">
                            <input type="text"
                                class="form-control @error("WLForm.websites.{$index}.link") is-invalid @enderror"
                                wire:model="WLForm.websites.{{ $index }}.link"
                                placeholder="e.g., https://linkedin.com/in/yourprofile">
                            @if ($index > 0)
                                <i class="bi bi-trash-fill btn rounded ms-1"
                                    wire:click="removeRow({{ $index }})"></i>
                            @endif
                        </div>
