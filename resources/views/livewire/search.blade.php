                ...
                @if (count($companies) > 0)
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <button class="btn slash
                            {{ $isFollowing ? 'btn-outline-danger' : ($isRequested ? 'btn-outline-warning' : 'btn-outline-primary') }}
                            btn-sm" wire:click="{{ !$isRequested ? ($isFollowing ? 'unFollow(' . $company['id'] . ')' : 'follow(' . $person['id'] . ')') : '' }}">
                                {{ $isFollowing ? 'UnFollow' : ($isRequested ? 'Requested' : 'Follow') }}
                            </button>
            
                            @if ($isFollowing)
                            <button wire:click.prevent="startConversation({{$company['id']}})" class="btn btn-outline-primary flex-shrink-0 p-1 w-50">Connect</button>
                            @endif
                        </div>
                    </li>
                    @endforeach
